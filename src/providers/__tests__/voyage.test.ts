...
describe('VoyageEmbeddingProvider', () => {
  ...
  it('passes input_type "document" for batch', async () => {
    ...
    const provider = new VoyageEmbeddingProvider('test-key');
    await provider.embedBatch(['a', 'b'], { model: 'voyage-4-lite', inputType: 'document' });
  ...
  it('omits input_type when not provided', async () => {
    ...
    const provider = new VoyageEmbeddingProvider('test-key');
    await provider.embed('hello', { model: 'voyage-4-lite' });
  ...
  it('handles batch embeddings in correct order', async () => {
    ...
    const provider = new VoyageEmbeddingProvider('test-key');
    const results = await provider.embedBatch(['a', 'b'], { model: 'voyage-4-lite' });
  ...
  it('returns empty array for empty batch', async () => {
    const provider = new VoyageEmbeddingProvider('test-key');
    const results = await provider.embedBatch([], { model: 'voyage-4-lite' });
  ...
  it('throws on non-OK response', async () => {
    ...
    const provider = new VoyageEmbeddingProvider('test-key');
    await expect(provider.embed('hi', { model: 'voyage-4-lite' })).rejects.toThrow('Voyage embeddings failed (500)');
  });
  ...
  it('throws when no data returned', async () => {
    ...
    const provider = new VoyageEmbeddingProvider('test-key');
    await expect(provider.embed('hi', { model: 'voyage-4-lite' })).rejects.toThrow('no data');
  });

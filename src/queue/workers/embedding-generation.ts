  ...
  const worker = new Worker<EmbeddingJobData>(
    ...
    async (job: Job<EmbeddingJobData>) => {
      ...
      const { result, durationMs } = await trackOperation(
        ...
        (res) => ({
          providerDurationMs: res.providerDurationMs ?? null,
      ...
      // Consolidated embedding log — replaces per-step info lines.
      logger.debug(
        {
          source: captureCallSite('job'),
          model: `${deps.embeddingProvider.name} | ${result.model}`,
          duration: formatDuration(durationMs, result.providerDurationMs),
          chars: formatLength(content.length),
          tokens: formatTokens(result.inputTokens, undefined),
          memoryRecordId,
        },
        'Embedding document',
      );
